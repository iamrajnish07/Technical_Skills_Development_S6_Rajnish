//Question 12: Convert infix expression to prefix using stack
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
int prec(char c){ if(c=='+'||c=='-') return 1; if(c=='*'||c=='/') return 2; if(c=='^') return 3; return 0; }
int isOperator(char c){ return (c=='+'||c=='-'||c=='*'||c=='/'||c=='^'); }
void reverseStr(char *s){
    int i=0,j=strlen(s)-1; while(i<j){ char t=s[i]; s[i]=s[j]; s[j]=t; i++; j--; }
}
int main(){
    char s[10005]; if(!fgets(s, sizeof(s), stdin)) return 0;
    int n=strlen(s); while(n>0 && (s[n-1]=='\n' || s[n-1]=='\r')) s[--n]=0;
    reverseStr(s);
    for(int i=0;i<n;i++) if(s[i]=='(') s[i]=')'; else if(s[i]==')') s[i]='(';
    char *stack = (char*)malloc(n);
    int top=-1;
    char out[10005]; int op=0;
    for(int i=0;i<n;i++){
        char c=s[i];
        if(c==' '||c=='\t') continue;
        if(isalnum(c)) out[op++]=c;
        else if(c=='(') stack[++top]=c;
        else if(c==')'){ while(top!=-1 && stack[top]!='(') out[op++]=stack[top--]; if(top!=-1) top--; }
        else if(isOperator(c)){
            while(top!=-1 && isOperator(stack[top]) && ((prec(stack[top])>prec(c)) || (prec(stack[top])==prec(c) && c!='^'))){
                out[op++]=stack[top--];
            }
            stack[++top]=c;
        }
    }
    while(top!=-1) out[op++]=stack[top--];
    out[op]=0;
    reverseStr(out);
    printf("%s\n", out);
    free(stack);
    return 0;
}